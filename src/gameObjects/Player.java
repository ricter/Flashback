...
public class Player extends GameObject {
    ...
    public void tryToFire() {
        ...
        if (gameScreen.millis() - lastFired > adjustedFireRate) { // fire bullet
            ...
            Physics.addPlayerBullet(new PlayerBullet(gameScreen,
                                                     this.bulletSpawnPosition.x + Flashback.levelData.getxDistanceFromLeftWall(), // need to scale because of the map
                                                     this.bulletSpawnPosition.y,
                                                     Flashback.bulletSprite,
                                                     gameScreen.mouseX + Flashback.levelData.getxDistanceFromLeftWall(), // need to scale because of the map
                                                     gameScreen.mouseY,
                                                     getCurrentTotalBPS()));
    ...
    public void update(float deltaT) {
        ...
        if (isGoUp()) {
            ...
            // TODO re-add ability to go down
        ...
        /* heartbeatTimer -= bps*deltaT; if(heartbeatTimer < 0) { heartbeatTimer = 2.0; if(dub != null) { lub.rewind(); lub.play(); } } else if( heartbeatTimer
         * < 1 && heartbeatTimer > .95) { if(lub != null) { dub.rewind(); dub.play(); } } */
